...
def softmax(feed):
    inputTensor = feed[0]
    diff = inputTensor - np.max(inputTensor)
    e_x = np.exp(diff)
    res = e_x / e_x.sum(axis=1)

    # print("[!] SOFTMAX:", res.tolist())
    # print(["angry", "disgust", "fearful", "happy", "neutral", "sad", "surprised"])
...
class FacialExpressionRecog:
    ...
    def infer(self, image, bbox=None):
        ...
        nmax = self._postprocess(outputBlob)
        return nmax, softmax(outputBlob)
